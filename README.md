...
- Comparative cloud model metadata and rank sanity checks should be cross-referenced against independent/current leaderboards before treating an unverified rank as authoritative: BenchLM (`https://benchlm.ai`), Artificial Analysis (`https://artificialanalysis.ai/leaderboards/models`), LLM Stats (`https://llm-stats.com/leaderboards/llm-leaderboard` and `/benchmarks`), WhatLLM (`https://whatllm.org/explore`), Vellum (`https://www.vellum.ai/llm-leaderboard`), LM Market Cap (`https://lmmarketcap.com`), Onyx (`https://onyx.app/llm-leaderboard`), Klu (`https://klu.ai/llm-leaderboard`), Aiven (`https://aiven.io/tools/llm-leaderboard`), Hugging Face ArtificialAnalysis leaderboard space, OpenRouter rankings (`https://openrouter.ai/rankings`), and the OpenRouter filtered text-output model list (`https://openrouter.ai/models?context=128000&fmt=cards&min_price=0.1&output_modalities=text`).
